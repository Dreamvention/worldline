...
class ControllerExtensionPaymentWorldline extends Controller {
	...
	public function index() {			
		...
		if ($setting['account']['api_key'][$environment] && $setting['account']['api_secret'][$environment] && !$this->callback() && !$this->webhook() && !$this->cron()) {
			...
			return $this->load->view('extension/payment/worldline/worldline', $data);
		}
				...
				if (!empty($hosted_checkout_response->getCreatedPaymentOutput())) {
					...
					if ($worldline_order_info) {
						...
						if (($transaction_status == 'cancelled') || ($transaction_status == 'rejected') || ($transaction_status == 'rejected_capture') || ($transaction_status == 'refunded')) {
							$this->response->redirect($this->url->link('extension/payment/worldline/failurePage', '', true));
						}
					}
				}
				
				$this->response->redirect($this->url->link('extension/payment/worldline/waitingPage', '', true));
			}
	...
	public function waitingPage() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/worldline/waitingPage', '', true)
		);
		...
		$this->response->setOutput($this->load->view('extension/payment/worldline/waiting_page', $data));
	}
	...
	public function pendingPage() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_title'),
			'href' => $this->url->link('extension/payment/worldline/pendingPage', '', true)
		);
		...
		$this->response->setOutput($this->load->view('extension/payment/worldline/pending_page', $data));
	}
	...
	public function failurePage() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_title'),
			'href' => $this->url->link('extension/payment/worldline/failurePage', '', true)
		);
		...
		$this->response->setOutput($this->load->view('extension/payment/worldline/failure_page', $data));
	}
	...
	public function getPaymentInfo() {
		if (!empty($this->request->post['order_id'])) {
			...
			if ($worldline_order_info) {
				...
				if (($transaction_status == 'cancelled') || ($transaction_status == 'rejected') || ($transaction_status == 'rejected_capture') || ($transaction_status == 'refunded')) {
					$data['redirect'] = $this->url->link('extension/payment/worldline/failurePage', '', true);
				}
				...
				if (!$data['redirect']) {
					$data['redirect'] = $this->url->link('extension/payment/worldline/pendingPage', '', true);
				}
			}
		}
